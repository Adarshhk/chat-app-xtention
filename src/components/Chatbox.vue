<template>
    <div class="w-full h-max bg-slate-300 rounded">
        ...
        <div class="w-full bg-gray-600 flex items-center rounded">
            ...
            <button @click="props.closeChatBox" class="rounded-lg hover:bg-gray-500 transition-all duration-300 mr-4 my-2 p-4">
        ...
        <div class="bg-slate-300 w-full h-96 overflow-y-auto p-4 space-y-4">
            ...
            <div v-else>
                ...
                >
                    <div :class="['max-w-[75%] px-4 py-3 rounded-lg overflow-hidden', chat.receiver_role !== profileStore.profileData.role ? 'bg-blue-600' : 'bg-gray-700']">
        ...
        <div class="bg-blue-400 h-12 rounded-sm flex items-center px-4 space-x-2">
            <input placeholder="Write a message..." type="text" class="flex-grow rounded-xl bg-white p-2 text-black focus:outline-none" v-model="message.content">
            <button :disabled="sendingMsg" @click="sendMessage" class="bg-blue-600 p-2 rounded-full hover:bg-blue-700 transition">
